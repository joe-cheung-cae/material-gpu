...
TEST_F(MaterialBuilderTest, ValidationErrors) {
    ...
    EXPECT_THROW(builder.no_contact().build(), std::runtime_error);
    
    // Test invalid elastic properties
    EXPECT_THROW(builder.elastic(-1.0f, 0.3f, 7850.0f).build(), std::invalid_argument);
    EXPECT_THROW(builder.elastic(2.1e11f, 0.6f, 7850.0f).build(), std::invalid_argument);
    EXPECT_THROW(builder.elastic(2.1e11f, 0.3f, -1000.0f).build(), std::invalid_argument);
}

TEST_F(MaterialBuilderTest, OptionalProperties) {
    // Test material without optional properties
    auto simple_material = builder
        .elastic(2.0e11f, 0.3f, 7800.0f)
        .no_contact()
        .build();
